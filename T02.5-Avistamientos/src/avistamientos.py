...
from coordenadas import Coordenadas, redondear
...
def lee_avistamientos(fichero):
    ...
    with open(fichero, encoding="utf-8") as f:
        ...
        for fechahora, ciudad, estado, forma, duracion, comentarios, latitud, longitud in lector:
            fechahora = parse_datetime(fechahora, "%m/%d/%Y %H:%M")
            duracion = int(duracion)
            coordenadas = Coordenadas(float(latitud), float(longitud))
            res.append(Avistamiento(fechahora, ciudad, estado, forma, duracion, comentarios, coordenadas))
        return res
...
def formas_estados2(avistamientos, estados):
    # Por comprensión
    conjunto_formas = {a.forma for a in avistamientos if a.estado in estados}
    return len(conjunto_formas)
...
def avistamientos_cercanos_ubicacion(avistamientos, ubicacion, radio):
    ...
    res = set()
    for av in avistamientos:
       #if disctancia_haversine(av.coordenadas, ubicacion)<radio:
            res.add(av)
    return res

def avistamientos_cercanos_ubicacion2(avistamientos, ubicacion, radio):
    ## Por compresión
    #return { av for av in avistamientos if disctancia_haversine(av.coordenadas, ubicacion)<radio}
    pass
...
def media_dias_entre_avistamientos(avistamientos, anyo=None):
    ...
    @rtype:-float
    '''    
    #Obtener las fechas de los avistamientos ordenas
    fecha_ord = sorted(av.fechahora.date() for av in avistamientos if anyo == None or av.fechahora.year == anyo)
    #Calcular los dias transcurridos entre una fecha y otra
    dias  = dias_entre_fechas(fecha_ord)
    #Calcular la media de los dias transcurridos
    num_elem = len(dias)
    res =None
    if num_elem>0:
        res = sum(dias)/len(dias)
        return res
...
def dias_entre_fechas(fechas):
    ...
    for fecha1, fecha2 in zip(fechas, fechas[1:]):
        ...
        res.append(dias_transcurridos)
    return res

def dias_entre_fechas2(fechas):
    ## Por compresión y con zip
    return [(fecha2-fecha1).days for fecha1, fecha2 in zip(fechas, fechas[1:])]

def dias_entre_fechas3(fechas):
    # Con range e índices
    res =[]
    for ind in range(len(fechas)-1):
        fecha1 = fechas[ind]
        fecha2 = fechas[ind+1]
        dias_transcurridos = (fecha2-fecha1).days
        res.append(dias_transcurridos)
...
def dias_entre_fechas4(fechas):
    ...
    return [(fechas[ind+1]-fechas[ind]).days for ind in range(len(fechas)-1)]
...
def hora_mas_avistamientos2(avistamientos):
    ...
    c = Counter( av.fechahour.hour for av in avistamientos)
...
def longitud_media_comentarios_por_estado(avistamientos):
    ...
    return { estado:longitud_media_comentarios(lista_avistamientos) for estado, lista_avistamientos in d.items()}
...
def longitud_media_comentarios(avistamientos):
    ...
    @rtype: float
    '''
    return statistics.mean(len(av.comentario) for av in avistamientos)
...
def porc_avistamientos_por_forma(avistamientos):  
    ...
    '''  
    d = contar_avistamientos_por_forma(avistamientos)
    total_avistamientos = len(avistamientos)
    return {forma:100*total_avis_forma/total_avistamientos for forma, total_avis_forma in d.items()}

def contar_avistamientos_por_forma(avistamientos):
    res = defaultdict(int)
    for av in avistamientos:
        res[av.forma] +=1
    return res
...
def porc_avistamientos_por_form2a(avistamientos):  
    ...
    d = Counter(av.forma for av in avistamientos)
    total_avistamientos = len(avistamientos)
    return {forma:100*total_avis_forma/total_avistamientos for forma, total_avis_forma in d.items()}
...
def avistamientos_mayor_duracion_por_estado(avistamientos, n=3):
    ...
    '''
    ##1. crear un diccionario de agrupacion, clave = estados, valores = lkistas de avistamientos de ese estado 
    d = agrupa_avistamientos_por_estado(avistamientos)
    ##2. Crear un diccionario a partir del anterior
    res = dict()
    for estado, lista_avistamiento in d.items():
        res[estado] = sorted(lista_avistamiento, key= lambda av:av.duracion, reverse=True)[:n]
    return res
...
def avistamientos_mayor_duracion_por_estado2(avistamientos, n=3):
    ...
    d = agrupa_avistamientos_por_estado(avistamientos)
    return {estado:sorted(lista_avistamiento, key= lambda av:av.duracion, reverse=True)[:n] for estado, lista_avistamiento in d.items()}
